...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        batchend = batch_size
        if batchend > len(unfinished_parses):
            batchend = len(unfinished_parses)
        transitions = model.predict (unfinished_parses[0:batchend])
        for i, up in enumerate(unfinished_parses[0:batchend]):
            up.parse_step(transitions[i])

        up_new = []
        for up in unfinished_parses:
            if (len(up.buffer) > 0) or (len(up.stack) != 1):
                up_new.append(up)

        unfinished_parses = up_new
